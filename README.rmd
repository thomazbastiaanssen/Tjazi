...
* First, since we're taking a natural logarithm, $\frac{{x}_{n}}{G({\bf x})}$ can never be zero as the logarithm of zero is undefined. This means that we need to either replace or remove every zero in our data before we use this transformation. We expand on strategies for this in the main text. 
...
* Third, if ${x}_{n}$ is exactly the same as the geometric mean ${G({\bf x})}$, $\frac{{x}_{n}}{G({\bf x})}$ will be 1 and thus $clr({x}_{n})$ will be 0 as the logarithm of 1 is equal to 0. This gives us some intuition about the size of CLR-transformed values. 
...
```{r Plot GBMs, message = FALSE, fig.width=16, fig.height=7}
...
```{r Plot GMMs, message = FALSE, fig.width=16, fig.height=20}
#Plot the features that show a group effect at q < 0.2
